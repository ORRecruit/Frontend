...
const SkillsInput: React.FC<SkillsInputProps> = ({ onSkillsChange, initialSkills }) => {
    ...
    <div>
      ...
      <div className="mt-2">
        {skills.map((skill, index) => (
          <div key={index}  className="inline-block bg-gray-200 text-gray-900 text-sm rounded px-6 py-2 mr-2 mb-2 relative">
            {skill}
            <button onClick={() => handleRemoveSkill(index)} className="ml-2 text-red-500 hover:text-red-700 absolute top-[2px] right-[2px]">
              <IoCloseSharp />
            </button>
          </div>
        ))}
